...
# s17_download_dragen_joined_copy_number_variants_IHCAPX8.sh
# Alexey Larionov, 29Nov2020

# Use:
# sbatch s17_download_dragen_joined_copy_number_variants_IHCAPX8.sh

# Multi-sampe VCF for Copy Number Variants
# expected total size <1GB, expected download time <10min

#SBATCH -J s17_download_dragen_joined_copy_number_variants_IHCAPX8
...
#SBATCH --ntasks=1
#SBATCH --time=1:00:00
#SBATCH --output=s17_download_dragen_joined_copy_number_variants_IHCAPX8.log
#SBATCH --qos=INTR
...
filter_term="Dragen Joint Genotyping Pipeline IHCAPX8 CNV 11/28/2020"

# Get dataset id
dataset_id=$($bs list datasets --filter-field="${filter_field}" --filter-term="${filter_term}" --field=Id --format=csv)

# Set folder for data
target_folder="/rds/project/erf33/rds-erf33-medgen/data/wgs_nov_2020/s02_IHCAPX8/s03_dragen_joined_genotyping/s03_copy_number_variants"
