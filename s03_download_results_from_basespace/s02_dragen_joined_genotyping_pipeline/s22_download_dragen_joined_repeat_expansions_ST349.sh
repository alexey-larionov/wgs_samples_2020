...
# s22_download_dragen_joined_repeat_expansions_ST349.sh
# Alexey Larionov, 29Nov2020

# Use:
# sbatch s22_download_dragen_joined_repeat_expansions_ST349.sh

# Multi-sampe VCF for Repeat Expansions
# expected total size <1GB, expected download time <10min

#SBATCH -J s22_download_dragen_joined_repeat_expansions_ST349
...
#SBATCH --ntasks=1
#SBATCH --time=1:00:00
#SBATCH --output=s22_download_dragen_joined_repeat_expansions_ST349.log
#SBATCH --qos=INTR
...
filter_term="Dragen Joint Genotyping Pipeline ST349 EH 11/28/2020"

# Get dataset id
dataset_id=$($bs list datasets --filter-field="${filter_field}" --filter-term="${filter_term}" --field=Id --format=csv)

# Set folder for data
target_folder="/rds/project/erf33/rds-erf33-medgen/data/wgs_nov_2020/s03_ST349/s03_dragen_joined_genotyping/s04_repeat_expansions"
